...
As the printed result from  the _results_ data frame shows, `domin` just runs `r 2^3 - 1` linear regressions with all the different combinations of the IVs and collects the $R^2$ value associated with each model. 

### Designating Complete Dominance

Given the results in the _results_ data frame, I will explain first how to determine complete dominance of two IVs--in this case, between _am_ and _carb_.  Complete dominance is computed by:
...
As compared to complete dominance which only used 2, conditional dominance statistics will use all 7 fit statistics computed in the 3 conditional dominance statistics produced for each IV.

The user can think of conditional dominance statistics as "window" statistics in which a "moving average" of statistics from the _results_ data frame are combined.  Specifically, at the specific order in question (i.e., number of IVs in the model), the conditional dominance statistic for that order will be the sum of the models at the order in question with the IV included less the sum of the models at the previous order with the IV omitted.  This sum is then divided by the number of models at the order in question . 
...
One way of thinking about the general dominance statistic is as a weighted average of all the fit statistics in the model.

Another way of thinking about the general dominance statistic is as the arithmetic average of all the conditional dominance statistics for an IV.
